...
@onready var defensive_back_3 = get_node("/root/GameScene/DefensiveBack3")
...
func engage():
	...
	var target_position = defensive_back_3.global_position + Vector2(0, 10)  

	# Calculate direction to the adjusted position
	var direction_to_db3 = (target_position - global_position).normalized()

	# Set the velocity
	velocity = direction_to_db3 * speed
...
func _on_detection_area_body_entered(body: Node2D) -> void:
	if body.is_in_group("football") and not quarterback.has_ball and not runningback.has_ball and not wide_receiver_1.has_ball and not wide_receiver_2.has_ball and not wide_receiver_4.has_ball:
		has_ball = true

...
func _physics_process(delta):
	...
	if not is_blocked and not game_scene.run_play and not wr2.has_ball and pre_cover_:
		pre_cover()
	elif not is_blocked and not wr2.has_ball and not rb.has_ball and not pre_cover_ and not game_scene.run_play and not football.past_los:
		cover()
	elif not is_blocked and football.football_thrown and wr2.has_ball:
		pursue_wr()
...
func cover():
	...
	var desired_distance = game_scene.db_2_cover_distance  # Adjust this value as needed
...
func blocked():
	if game_scene.run_play:
		...
		# reduce movement
		linear_velocity = direction_to_wr2 * blocked_speed

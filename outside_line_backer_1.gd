...
func _physics_process(delta):
	...
	if not is_blocked and game_scene.outside_linebacker_1_play == 0:
		blitz()

	if not is_blocked and drop_coverage_bool and not in_coverage_bool and not football.football_thrown and not football.past_los and game_scene.outside_linebacker_1_play == 1:
		drop_coverage()
